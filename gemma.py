...
from LLMPruner.pruner import hf_gemma_pruner as gemma_pruner
...
def main(args):
    ...
    if pruner_type == 'random':
        ...
        imp = gemma_pruner.MagnitudeImportance(p=1)
    elif pruner_type == 'l2':
        imp = gemma_pruner.MagnitudeImportance(p=2)
    elif pruner_type == 'taylor':
        imp = gemma_pruner.TaylorImportance(group_reduction=args.grouping_strategy, taylor=args.taylor)
    else:
    ...
    if args.block_wise:
        kwargs = {
            ...
            "customized_pruners": {
                GemmaRMSNorm: gemma_pruner.gemma_rmsnorm_pruner,
                Gemma2RMSNorm: gemma_pruner.gemma_rmsnorm_pruner,
            },
        ...
        for i in range(args.iterative_steps):
            ...
            model.config.num_key_value_heads = model.model.layers[args.block_attention_layer_start].self_attn.k_proj.weight.data.shape[0] // layer.self_attn.head_dim
    ...
    ckpt = torch.load(logger.best_checkpoint_path)
